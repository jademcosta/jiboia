...
func RegisterIngestingRoutes(api *Api, c *config.Config, flow domain.DataFlow) {
	api.mux.Post(fmt.Sprintf("/%s/async_ingestion", c.Flow.Name), asyncIngestion(api.log, flow))
}

func asyncIngestion(l *zap.SugaredLogger, flow domain.DataFlow) http.HandlerFunc {
